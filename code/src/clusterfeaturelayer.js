define([
    ...
    return declare([GraphicsLayer], {
        constructor: function(options) {
            ...
            //console.log(this._getServiceDetails(), "test")
...
/*
        var timeOutAction;
            //reset previous timeout that would call for the popup
        if (typeof (window.timeOutAction) !== null){
                //if timeOutAction is not null or undefined, clear it out
                clearTimeout(timeOutAction);
         }
            
            //use setTimeOut to urn the function that shows the popup after 1 sec
...
*/
        onMouseMove: function(e) {
            ...
            if (e.graphic.attributes.clusterCount === 1 ) {
                ...
                // Remove info graphics from layer
                this.clearSingles(this._singles);
                //console.log(this._singles)
                ...
                if (!this._disablePopup) {
                    ...
                    //this._map.infoWindow.show(e.graphic.geometry);
                    this._newClusterDiv(this._singles);
                }
            }
        ...
        onClick: function(e){
            ...
            if (this._zoomOnClick && e.graphic.attributes.clusterCount > 1 && this._map.getZoom() !== this._map.getMaxZoom())
            {
                // Zoom to level that shows all points in cluster, not necessarily the extent
                var extent = this._getClusterExtent(e.graphic);
                if (extent.getWidth()) {
        ...
        _newClusterDiv: function(clusterArray){
            var clusterDiv = document.getElementById("clusterAttributes");
            clusterDiv.style.display = 'block';
            clusterDiv.innerHTML = '';
            for (i = 0; i < clusterArray.length; i++) {
                //console.log(clusterArray.length, i)
                if(clusterArray.length - 1 == i){
                    var clusterInfo = "<b>Name:</b> " + clusterArray[i].attributes.First_Name + " " + clusterArray[i].attributes.Last_Name + "<br>" +
                    "<b>Affiliation: </b>" + clusterArray[i].attributes.Affiliatio + "<br>" +
                    "<b>Institution: </b>" + clusterArray[i].attributes.Institutio + "<br>" +
                    "<b>Start Date: </b>" + clusterArray[i].attributes.Start_Date +"<br>" +
                    "<b>End Date: </b>" + clusterArray[i].attributes.End_Date;
                }
                else{                
                    var clusterInfo = "<b>Name:</b> " + clusterArray[i].attributes.First_Name + " " + clusterArray[i].attributes.Last_Name + "<br>" +
                    "<b>Affiliation: </b>" + clusterArray[i].attributes.Affiliatio + "<br>" +
